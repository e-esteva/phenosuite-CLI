def n_wise_logOdds(spatial_obj,out_dir,label,draw=False,resolution=0.3774,p1=3,p2=30,compute_effect_size=False,target_celltypes=None):
  import numpy as np
  import pandas as pd
  import scipy
  ...
  set_sizes=spatial_obj['celltype'].value_counts()
  celltypes=set_sizes.keys()

  if target_celltypes is None:
    target_celltypes=celltypes
  ...
  # outer loop over targets (if defined, otherwise all celltypes)
  for i in range(len(target_celltypes)):
  
    neighbors_i=[]
  
    ref_=target_celltypes[i]
    ref_cl=set_sizes[ref_] > c_l
    ref_cl_ratio=set_sizes[ref_]/c_l
    ref_data=spatial_obj[['x','y']][(spatial_obj['celltype']==ref_)]
    ...
    for j in range(len(celltypes)):
      ...
      query_data=spatial_obj[['x','y']][(spatial_obj['celltype']==query_)]
    ...
    print('neighbors_i: '+str(neighbors_i))
    global_neighbors.append(neighbors_i)
  ...
  ###################################################  
  global_neighbors_df=pd.DataFrame(global_neighbors).T
  global_neighbors_df.columns=target_celltypes
  global_neighbors_df.index=celltypes
      
  global_neighbors_df.to_csv(str(out_dir)+'/'+str(label)+'-logOdds_matrix.csv')
  
  # pull target neighbor counts:
  target_celltype_idx = [list(celltypes).index(x) for x in target_celltypes]
  print('target celltype idx')
  print(target_celltype_idx)
  # sum of target interactions:
  ## for every target celltype find neighbor counts for other targets:
  print(global_neighbors_df[global_neighbors_df.columns[0]])
  print(np.array([target_celltype_idx[x] for x in [1,2]]))
  print(global_neighbors_df[global_neighbors_df.columns[0]][np.array([target_celltype_idx[x] for x in [1,2]])])
  print(np.sum(global_neighbors_df[global_neighbors_df.columns[0]][np.array([target_celltype_idx[x] for x in [1,2]])]))
  print([np.array([target_celltype_idx[y] for y in range(len(target_celltype_idx)) if y != x]) for x in range(len(target_celltypes))])
  target_neighbor_interactions_idx = [np.array([target_celltype_idx[y] for y in range(len(target_celltype_idx)) if y != x]) for x in range(len(target_celltypes))]
  
  #target_neighbor_interactions = np.sum([np.sum(global_neighbors_df[global_neighbors_df.columns[x]][np.array([target_celltype_idx[y] for y in range(len(target_celltype_idx)) if y != x])] for x in range(len(target_celltypes)))])
  target_neighbor_interactions = np.sum([np.sum(global_neighbors_df[global_neighbors_df.columns[x]][target_neighbor_interactions_idx[x]]) for x in range(len(target_celltype_idx))])
  # sums columns to get all target interactions, then sum over result:
  total_target_interactions = np.sum(global_neighbors_df.sum(axis=0))
  # empirical odds of interaction:
  print('target neighbor interactions')
  print(target_neighbor_interactions)
  print('total target interactions')
  print(total_target_interactions)
  n_simplex_emprical_odds=target_neighbor_interactions/total_target_interactions
  # random odds of interaction:
  ## cumulative probability of finding any member of the targets * combinatorial probability of finding both of the other n-1
  from itertools import combinations
  combinations_ = list(combinations([x for x in range(len(target_celltypes))], len(target_celltypes)-1))
  target_probabilities = np.array([set_sizes[x]/spatial_obj.shape[0] for x in target_celltypes])
  # Bayesian framing: P(A|B) = P(B|A)*P(A)/P(B)
  # event A: sampling n-1 / n events
  # event B: sampling 1 event first
  # P(B|A) = 1; P(A) = combinations_; P(B) = cum_target_probability
  combination_probabilities=np.sum([np.prod(target_probabilities[list(x)]) for x in combinations_])
  n_simplex_theoretical_odds= (combination_probabilities)/np.sum(target_probabilities)
  print('empirical odds: '+str(n_simplex_emprical_odds))
  print('theoretical odds: '+str(n_simplex_theoretical_odds))
  log_odds_global = np.log(np.exp(n_simplex_emprical_odds)/np.exp(n_simplex_theoretical_odds))
  # probabilities:
  global_probabilities_df=pd.DataFrame(global_probabilities).T
  global_probabilities_df.columns=target_celltypes
