...
def PreProcessImage(image_source,spatial_metadata,adjust_coords=True):
	# import qptiff:
	image = skimage.io.imread(fname=str(image_source),plugin='tifffile')
	...
	return image,raw_img_size, set_subset_x,set_subset_y



def get_mask_channels(image,spatial_metadata,raw_img_size,target_size=4,adjust_coords=True):
	# binning spatial metadata by cluster:
	binned_metadata=[metadata[metadata['cluster']==x] for x in list(set(metadata['cluster']))]

	channels = {}
	for cluster in binned_metadata:
		if cluster.shape[0] > 1:
			...
			spatial_data_cluster_x1=[int(x)+1 for x in cluster_x]
			spatial_data_cluster_x2=[int(x)-1 for x in cluster_x]

			spatial_data_cluster_x0.extend(spatial_data_cluster_x1)
			spatial_data_cluster_x0.extend(spatial_data_cluster_x2)
			spatial_data_cluster_x=np.array(spatial_data_cluster_x0)

			spatial_data_cluster_y0=[int(x) for x in cluster_y]
			spatial_data_cluster_y1=[int(x)+1 for x in cluster_y]
			spatial_data_cluster_y2=[int(x)-1 for x in cluster_y]

			spatial_data_cluster_y0.extend(spatial_data_cluster_y1)
			spatial_data_cluster_y0.extend(spatial_data_cluster_y2)
			spatial_data_cluster_y=np.array(spatial_data_cluster_y0)
			...
			image_tmp=image
			masked_image=np.sum(image_tmp,axis=0)
			mask = np.ones(shape=image_tmp.shape[1:3], dtype="bool")
			mask[spatial_data_cluster_y_exp-1,spatial_data_cluster_x_exp-1]=False
			masked_image[mask]=0
			exp_mag=np.sum(masked_image)

			# all mask channels are the same size so:
			cluster_idx=list(set(metadata['cluster'])).index(list(set(cluster['cluster']))[0])
			if cluster_idx == 0:
				mask_size=(np.array(masked_image,dtype='uint8').nbytes/1e9)*len(binned_metadata)
				compression_factor=np.sqrt(target_size/(mask_size+raw_img_size))
			print('compression factor: '+str(compression_factor))

			# pre-filtering discrete values:
			masked_image_preFiltered=ndimage.interpolation.spline_filter1d(masked_image)
			masked_image=ndimage.interpolation.zoom(masked_image_preFiltered,compression_factor)
			cluster_channel=str(cluster_id)+'_mask-expanded'

			print('Appending expanded image')
			channels[cluster_channel]=masked_image
			print(np.sum(image))
...
def compress_marker_channels(channels, compression_factor,spatial_metadata,relevant_markers=None,adjust_coord=True):
	channel_names=[]
	with TiffFile(image_source) as tif:
		for page in tif.series[0].pages:
			...
			if page_name_element is not None:
				...
				if relevant_markers is not None:	
					markers_use=relevant_markers['x']
					...
					if page_name in markers_use or page_name in markers_mod:
						...
						page_tmp=ndimage.interpolation.zoom(page_tmp,compression_factor)
...
def writeMaskTiff(channels,outPath):
	...
	# write new tiff:
	tifffile.imsave( str(outPath),new_tiff ,imagej=True,ijmetadata=ijmetadata)
...
def get_circle_masks(image,metadata,set_subset_x,set_subset_y,filled=True,radius=10,num_points=100,adjust_coords=1,compress=False,relevant_markers=None,mapping=None,skip_classes=None,target_size=4,filter=False):
	import tqdm
	...
	for cluster in tqdm.tqdm(binned_metadata):
		if cluster.shape[0] > 1:
			...
			if filled:
				circle_coords = [generate_filled_circle(cx=spatial_data_cluster_x0[x],cy=spatial_data_cluster_y0[x],r=radius) for x in range(len(spatial_data_cluster_x0))]
				...
				circle_coords = [generate_circle(x_center=spatial_data_cluster_x0[x],y_center=spatial_data_cluster_y0[x],radius=radius,num_points=num_points) for x in range(len(spatial_data_cluster_x0))]
		...
		#masks[cluster_channel]=mask
        
		# all mask channels are the same size so:
		cluster_idx=list(set(metadata['cluster'])).index(list(set(cluster['cluster']))[0])
		if cluster_idx == 0 and compress:
			mask_size=(np.array(mask,dtype='uint8').nbytes/1e9)*len(binned_metadata)
			compression_factor=np.min([1,np.sqrt(target_size/(mask_size+raw_img_size))])
			print('compression factor: '+str(compression_factor))
		...
		if compress:
			if filter:
				...
				masked_image_filtered=ndimage.interpolation.zoom(masked_image_preFiltered,compression_factor)
				channels[cluster_channel]=masked_image_filtered
			else:
				# compressed, no filtering
				masked_image_nf=ndimage.interpolation.zoom(mask,compression_factor)
...
def write_ome_bigTiff(channels,out,channels_to_keep):
	...
	tifffile.imsave( out,new_tiff ,metadata={"Channel": {"Name": labs[0]}},ome=True,bigtiff=True)

	return None


def get_continuous_channels(image_source,channels,set_subset_x=None,set_subset_y=None,relevant_markers=None,adjust_coords=1):
	if relevant_markers is not None:
		relevant_markers=pd.read_csv(str(relevant_markers))
		markers_use=relevant_markers['x']
		markers_mod=["".join(re.split('_',x)) for x in markers_use]
		markers_mod_tmp=[re.sub('_','-',x) for x in markers_use]
		markers_mod.extend(markers_mod_tmp)
